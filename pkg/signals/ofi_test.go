...
func TestOFIStep_NoChange(t *testing.T) {
	...
	if e := OFIStep(s, s); !e.IsZero() {
		t.Errorf("no-change OFI = %s, want 0", e)
	}
}

func TestOFIStep_BidQtyUp(t *testing.T) {
	prev := snap([][2]string{{"100", "5"}}, [][2]string{{"101", "5"}})
	cur := snap([][2]string{{"100", "8"}}, [][2]string{{"101", "5"}})
	// Same bid price, +3 size; ask unchanged. e = (8-5) - 0 = +3.
	if e := OFIStep(prev, cur); !e.Equal(dec("3")) {
		t.Errorf("bid-qty-up OFI = %s, want 3", e)
	}
}

func TestOFIStep_AskQtyUp(t *testing.T) {
	prev := snap([][2]string{{"100", "5"}}, [][2]string{{"101", "5"}})
	cur := snap([][2]string{{"100", "5"}}, [][2]string{{"101", "9"}})
	// Same ask price, +4 size (more sell pressure). e = 0 - (9-5) = -4.
	if e := OFIStep(prev, cur); !e.Equal(dec("-4")) {
		t.Errorf("ask-qty-up OFI = %s, want -4", e)
	}
}

func TestOFIStep_BidPriceUp(t *testing.T) {
	prev := snap([][2]string{{"100", "5"}}, [][2]string{{"102", "5"}})
	cur := snap([][2]string{{"101", "3"}}, [][2]string{{"102", "5"}})
	// Bid stepped up: fresh buy size = current bid qty (3). Ask unchanged. e = +3.
	if e := OFIStep(prev, cur); !e.Equal(dec("3")) {
		t.Errorf("bid-price-up OFI = %s, want 3", e)
	}
}

func TestOFIStep_AskPriceDown(t *testing.T) {
	prev := snap([][2]string{{"100", "5"}}, [][2]string{{"102", "4"}})
	cur := snap([][2]string{{"100", "5"}}, [][2]string{{"101", "2"}})
	// Ask stepped down: fresh sell size = current ask qty (2). e = 0 - 2 = -2.
	if e := OFIStep(prev, cur); !e.Equal(dec("-2")) {
		t.Errorf("ask-price-down OFI = %s, want -2", e)
	}
}

func TestOFI_Accumulator(t *testing.T) {
	o := NewOFI()
	s0 := snap([][2]string{{"100", "5"}}, [][2]string{{"101", "5"}})
	s1 := snap([][2]string{{"100", "8"}}, [][2]string{{"101", "5"}}) // +3
	s2 := snap([][2]string{{"100", "8"}}, [][2]string{{"101", "9"}}) // -4
...
func TestOFI_FromRealBook(t *testing.T) {
	...
	add := func(user string, side types.Side, price, qty string) {
	...
	if e := OFIStep(before, after); !e.IsPositive() {
		t.Errorf("adding bid size should give positive OFI, got %s", e)
	}
}
