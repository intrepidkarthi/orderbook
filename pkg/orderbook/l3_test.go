...
func TestSnapshotL3_OrderByOrder(t *testing.T) {
	...
	if snap.Bids[0].OrderID != a.ID || snap.Bids[0].UserID != "alice" {
		t.Errorf("first bid = %s/%s, want alice's order", snap.Bids[0].UserID, snap.Bids[0].OrderID)
	}
	if snap.Bids[1].OrderID != b.ID {
		t.Error("second bid should be bob's (time priority)")
	}
	if snap.Bids[2].OrderID != c.ID || !snap.Bids[2].Price.Equal(dec("99")) {
	...
	if !snap.Bids[1].Quantity.Equal(dec("3")) {
		t.Errorf("bob's L3 qty = %s, want 3", snap.Bids[1].Quantity)
	}
}
