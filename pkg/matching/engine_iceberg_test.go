...
func iceberg(t *testing.T, user string, side types.Side, price, total, display string) *types.IcebergOrder {
	t.Helper()
	ib, err := types.NewIcebergOrder(lim(t, user, side, price, total), dec(display))
	if err != nil {
...
func TestIceberg_ShowsOnlyDisplaySlice(t *testing.T) {
	...
	if _, qty, ok := e.BestBid(); !ok || !qty.Equal(dec("3")) {
		t.Errorf("visible best bid qty = %s, want 3 (rest hidden)", qty)
	}
}
...
func TestIceberg_RefillsAsConsumed(t *testing.T) {
	...
	if _, qty, ok := e.BestBid(); !ok || !qty.Equal(dec("1")) {
		t.Errorf("visible qty after 9 consumed = %s, want 1", qty)
	}
...
func TestIceberg_ImmediateCrossRefills(t *testing.T) {
	...
	if !total.Equal(dec("8")) {
		t.Errorf("iceberg bought %s, want 8", total)
	}
	// 1 unit of the 101 ask remains.
	if ask, qty, ok := e.BestAsk(); !ok || !ask.Equal(dec("101")) || !qty.Equal(dec("1")) {
		t.Errorf("remaining ask = %s x %s, want 101 x 1", ask, qty)
	}
}
