...
func marketOrder(t *testing.T, user string, side types.Side, qty string) *types.Order {
	t.Helper()
	o, err := types.NewOrder(user, "BTC-USD", side, types.OrderTypeMarket, decimal.Zero, dec(qty), types.TIFImmediateOrCancel)
	if err != nil {
...
func stopOrder(t *testing.T, user string, side types.Side, qty, stopPrice string) *types.StopOrder {
	t.Helper()
	s, err := types.NewStopOrder(marketOrder(t, user, side, qty), dec(stopPrice))
	if err != nil {
...
func TestStopLimit_RestsAsLimitOnTrigger(t *testing.T) {
	...
	underlying, _ := types.NewOrder("trader", "BTC-USD", types.SideSell, types.OrderTypeLimit, dec("97"), dec("4"), types.TIFGoodTillCancel)
	so, _ := types.NewStopOrder(underlying, dec("95"))
	e.ProcessStop(so)
	...
	if ask, _, ok := e.BestAsk(); !ok || !ask.Equal(dec("97")) {
		t.Errorf("expected a resting ask at 97 from the stop-limit, got %s (ok=%v)", ask, ok)
	}
}
