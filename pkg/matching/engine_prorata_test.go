...
func sumQty(trades []*types.Trade) decimal.Decimal {
	s := decimal.Zero
...
func TestProRata_AllocatesProportionally(t *testing.T) {
	...
	if !res.Trades[0].Quantity.Equal(dec("5")) || !res.Trades[1].Quantity.Equal(dec("15")) {
		t.Errorf("allocations = %s, %s; want 5, 15", res.Trades[0].Quantity, res.Trades[1].Quantity)
	}
	// Both makers remain, level now holds 20 (40 − 20).
	if _, qty, _ := e.BestAsk(); !qty.Equal(dec("20")) {
		t.Errorf("level qty after = %s, want 20", qty)
	}
}
...
func TestProRata_FullConsume(t *testing.T) {
	...
	if !sumQty(res.Trades).Equal(dec("40")) {
		t.Errorf("total filled = %s, want 40", sumQty(res.Trades))
	}
...
func TestProRata_RemainderSumsExactly(t *testing.T) {
	...
	if !sumQty(res.Trades).Equal(dec("10")) {
		t.Errorf("total filled = %s, want exactly 10", sumQty(res.Trades))
	}
