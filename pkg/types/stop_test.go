...
func TestNewStopOrder_Validation(t *testing.T) {
	...
	if _, err := NewStopOrder(nil, dec("10")); !errors.Is(err, ErrNilOrder) {
		t.Errorf("nil order err = %v, want ErrNilOrder", err)
	}
	if _, err := NewStopOrder(o, dec("0")); !errors.Is(err, ErrInvalidStopPrice) {
...
func TestStopOrder_TriggerDirection(t *testing.T) {
	...
	ss, _ := NewStopOrder(sell, dec("95"))
	if ss.ShouldTrigger(dec("96")) {
		t.Error("sell stop should NOT trigger above stop")
	}
	if !ss.ShouldTrigger(dec("95")) {
		t.Error("sell stop should trigger at stop")
	}
	if !ss.ShouldTrigger(dec("94")) {
	...
	bs, _ := NewStopOrder(buy, dec("105"))
	if bs.ShouldTrigger(dec("104")) {
		t.Error("buy stop should NOT trigger below stop")
	}
	if !bs.ShouldTrigger(dec("105")) {
