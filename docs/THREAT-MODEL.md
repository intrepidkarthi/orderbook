...
| 16 | **Asymmetric ingress speed bump on takers** | Latency arb (Flash Boys / IEX). Auction wiring (#7) covers it structurally. | L | **Gateway** | ⚠️ **Observation hook only.** `gateway.Gateway.OnBump` identifies liquidity-taking orders and reports when they *would* be released, then forwards them immediately — nothing is delayed. Real hold-and-release needs the embedder's scheduler and wall clock. Use the batch auction (#7) for a structural answer. |
