...
| `MaxMarkStep` | `decimal.Decimal` | Reject a `SetMarkPrice` update jumping more than this fraction from the current mark (`ErrMarkStepTooLarge`) | `0` (disabled) | anti oracle-pump (single jump); first mark and clearing to 0 always accepted |
| `MinMarkDepth` | `int64` | Reject a `SetMarkPrice` move to a price not backed by this many resting lots within `MarkDepthBand` (`ErrMarkDepthTooThin`) | `0` (disabled) | anti oracle-pump (patient drag); depth-backed complement to `MaxMarkStep` |
| `MarkDepthBand` | `decimal.Decimal` | Fraction around a proposed mark within which `MinMarkDepth` depth is counted | `0` → `MaxMarkStep`, then `PriceBand`, else whole book | e.g. `0.02` = ±2% |
| `IcebergPeakJitter` | `decimal.Decimal` | Vary each refilled iceberg slice by up to ±this fraction of the display size | `0` (fixed peak) | anti-sniffing; deterministic in (order id, refill count) — replay-exact |
...
- **Mark guards** (`MaxMarkStep`, `MinMarkDepth`/`MarkDepthBand`) — `SetMarkPrice`
  returns an error and rejects an outsized single jump (`MaxMarkStep`) *and* a move
  to a price the resting book does not back with real depth (`MinMarkDepth`), so a
  thin-book oracle pump cannot lead the mark, whether in one jump or a patient
  in-step drag.
- **Iceberg peak jitter** (`IcebergPeakJitter`) — varies each hidden-reserve reload
  size deterministically, so a watcher cannot fingerprint an iceberg by its fixed
  reload.
...
**Beyond the core**, companion packages address the same threat catalogue:
...
`RampingDetector`, `PingingDetector`, and `CrossBookMonitor` alongside the spoof
and rate detectors); and `pkg/gateway` (an *enforcing* token-bucket `RateGate`, a
taker speed bump, plus a CAT-style audit export in `examples/gateway`) — the
controls that correctly live in the layer above a neutral, deterministic matcher.
